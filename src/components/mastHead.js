...
import { Flex, Box, BorderBox } from '@primer/components'
...
function mastHead({ metaData }) {
  ...
  const MetaData = props =>
    layout === 'stacked' ? (
      <Box md={'inline-block'} mr={3} {...props} />
    ) : (
  ...
  return (
    <>
      ...
      {user.isDeveloperProgramMember && (
        <MetaData className={(style === 'dark' && 'text-white') || ''}>
          <span style={{ color: theme.iconColor }}>
            <Octicon
              icon={CircuitBoard}
              size={20}
          ...
          <span className="d-inline-block f5 rounded-2 text-white bg-green py-1 px-2">
      ...
      <div className="f4 mb-6">
        ...
        <hr className={`${(style === 'dark' && styles.darkHr) || ''}`} />
        <DarkModeToggle style={style} onThemeChange={onThemeChange} />
        <div className={'mb-1'}>
