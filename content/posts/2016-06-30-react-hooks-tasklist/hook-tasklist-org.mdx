---
  title: Writing a task list with react hooks
...
Feel free to take a moment to look into the
`React context api`
...
Now we need a heading to display how many 
tasks are in the list for the user
...
ok now for the last 2 components, 
which thankfully are very simple:
...
and finally our custom
text component to disply the
task text  
